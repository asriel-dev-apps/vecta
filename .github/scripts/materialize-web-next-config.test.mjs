...
const script = path.resolve(import.meta.dirname, "materialize-web-next-config.mjs");
...
/** A built config shaped like what `react-router build` emits for web-next. */
...
async function run(env, config = builtConfig()) {
  const root = await mkdtemp(path.join(tmpdir(), "vecta-web-next-config-"));
  const configPath = path.join(root, "wrangler.json");
  await writeFile(configPath, JSON.stringify(config));
  try {
    const result = await execute(process.execPath, [script], {
      env: { ...process.env, ...env, WEB_NEXT_CONFIG: configPath },
    });
...
test("materializes every placeholder var, the namespace ids, and the worker name", async () => {
  ...
  assert.match(result.stdout, /web_next_config_materialized/u);
