 ...
 * (`apps/web-next/build/server/wrangler.json`), never into the tracked source.
 *
 * `apps/web-next/wrangler.jsonc` deliberately ships `.invalid` placeholders so
...
const CONFIG_PATH =
  process.env.WEB_NEXT_CONFIG ?? "apps/web-next/build/server/wrangler.json";
...
console.log(
  JSON.stringify({
    event: "web_next_config_materialized",
    worker: config.name,
